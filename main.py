...
async def get_mal_info(mal_link=None, title=None):
    if mal_link:
...
async def process_mal_info(shows):
    tasks = []
    for show in shows:
        tasks.append(get_mal_info(show["mal_link"], show["name"]))
...
for day, shows in schedule.items():
    ...
    # Fetch MAL info for all shows in parallel
...
upcoming_events = []
loop = asyncio.get_event_loop()
upcoming_mal_infos = loop.run_until_complete(process_mal_info(upcoming))

for item, mal_info in zip(upcoming, upcoming_mal_infos):
    if item["date"]:
