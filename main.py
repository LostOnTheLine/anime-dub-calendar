...
# Load shows from YAML file
def load_shows():
    try:
        with open("shows.yaml", "r") as f:
            return yaml.safe_load(f) or {"ongoing": {}, "upcoming": []}
    except FileNotFoundError:
        return {"ongoing": {}, "upcoming": []}

# Save shows to YAML file
def save_shows(data):
    with open("shows.yaml", "w") as f:
...
try:
    ...
    show_data = load_shows()
    ongoing_data = show_data.get("ongoing", {})
    upcoming_data = show_data.get("upcoming", [])
